...
const min = 60 * 1000
const ether = 1e18
const faucetAmountWei = (0.01 * ether)
const EtherBN = new BN('1000000000000000000', 10)
const MAX_BALANCE = ethers.utils.parseEther("0.01")
  ...
  async function handleRequest (req, res) {
    try {
      ...
      const balanceTooFull = balance.gt(MAX_BALANCE)
       console.log(`Max Balance: ${MAX_BALANCE} ${balance}`);
      if (balanceTooFull) {
        console.log(`${requestorMessage} - already has too much ether`)
        return didError(res, new Error('User is greedy - already has too much XTH'))
      }
      const send = async () => {
        const tx = {
           to: targetAddress,
